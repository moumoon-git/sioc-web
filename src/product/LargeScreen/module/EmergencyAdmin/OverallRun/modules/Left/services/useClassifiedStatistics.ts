  ...
  const getData = (params?: { year: number, quarter: number}): void => {
    ...
    $http(request).then((response) => {
      if (response.code === 0 || response.errorcode === 0) {
        ...
        $message.error(`获取分类统计失败，错误代码${response.code ?? response.errorcode}，错误信息：${response.msg ?? response.message}`);
      }
    }).catch((error: Error) => {
      $message.error(`获取分类统计失败，错误信息：${error}`);
    });
  };
  getData();
  return [
