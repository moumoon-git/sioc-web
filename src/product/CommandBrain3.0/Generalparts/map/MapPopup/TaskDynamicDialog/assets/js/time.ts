 ...
 *
 * @param {string} time 时间
 * @param formate 要得到的时间格式
 * @returns 返回指定的时间格式
 * @description 计时器
 */
function setCalculagraph(time: any, formate: any) {
  let result = getTimeDiff(time)
  // let result = formatDate(formate, time);
  return result;
}
 ...
 * @returns 
 */
function getTimeDiff(diffTime: number) {
  ...
  if (diffTime > 0) {
    ...
    return diffd > 0
      ? addZero(diffd) + '天' + addZero(diffh) + '时' + addZero(diffm) + '分' + addZero(diffs) + '秒'
      : addZero(diffh) + '时' + addZero(diffm) + '分' + addZero(diffs) + '秒';
    // return addZero(diffd) + '天' + addZero(diffh) + '时' + addZero(diffm) + '分' + addZero(diffs) + '秒'
  } else {
    return '00:00:00';
  }
}
