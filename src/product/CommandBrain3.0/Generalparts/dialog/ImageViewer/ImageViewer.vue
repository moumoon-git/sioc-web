<template>
  <teleport to="body">
    <div v-show="showImagePopup" :key="String(Math.random()*1000)" class="image-viewer-wrap">
      <!-- 最小化 -->
      <div v-if="hide" :class="$style.imageViewerMini" ref="imageViewerMini">
        <header
          @mousedown="handleMouseDown"
          @mouseup="handleMouseUp"
          :class="$style.imageViewerHeaderMini"
        >
          <div :class="$style.imageViewerHeaderLeftMini">{{ title }}</div>
          <div :class="$style.imageViewerHeaderRight">
            <button
              v-if="fix"
        ...
        <main :class="$style.imageViewerMainMin">
          <img :src="path" alt="" :class="$style.image" />
          <img src="./assets/svg/picture.svg" alt="" :class="$style.icon" />
        </main>

        <footer v-if="contactor" :class="$style.imageViewerFooterMini">
          <div>
      ...
      <!-- 正常大小 -->
      <div v-else :class="$style.imageViewer" :key="String(Math.random()*1000)" ref="imageViewer">
        <header
          @mousedown="handleMouseDown"
          @mouseup="handleMouseUp"
          :class="$style.imageViewerHeader"
        >
          <div :class="$style.imageViewerHeaderLeft">{{ title }}</div>
          <div :class="$style.imageViewerHeaderRight">
        ...
        <main :class="$style.imageViewerMain">
          <img :src="path" alt="" :class="$style.image" />
        </main>

        <footer v-if="contactor" :class="$style.imageViewerFooter">
          <div>
...
export default defineComponent({
  ...
  setup() {
    ...
    instance!.$imageViewer = (options?: { path?: string; title?: string; contactor?: object; address?: string; }) => {
      console.log(options);
  ...
  methods: {
    ...
    handleMouseDown(event: MouseEvent) {
      ...
      const dialog = this.hide
        ? (this.$refs.imageViewerMini as HTMLElement)
        : (this.$refs.imageViewer as HTMLElement);
